...
def find_text_boxes(pre, min_text_height_limit=10, max_text_height_limit=100):
    pre = cv2.Canny(pre, 30, 200)
...
def find_table_in_boxes(boxes, cell_threshold=15, min_columns=2):
    rows = {}
    cols = {}
    for box in boxes:
        (x, y, w, h) = box
...
if __name__ == '__main__':
    ...
    aligned_img_threshold = cv2.threshold(aligned_img_grey, 90, 255, cv2.THRESH_BINARY | cv2.THRESH_OTSU)[1]
    ...
    aligned_img_mask = np.zeros_like(aligned_img_show)
    cv2.rectangle(aligned_img_mask, (30, 300), (730, 620), (0, 0, 255), cv2.FILLED)
    aligned_img_show = cv2.addWeighted(aligned_img_show, 0.99, aligned_img_mask, 0.1, 0)
    img = img_cropped[300:620, 30:730]
